const requestURL =
  "https://raw.githubusercontent.com/isaac1125/BoardgameInfoWeb/master/list.json";
...
function print(boardgameInfo) {
  ...
  document.querySelector(".heading__primary").textContent =
    boardgame.boardgameName[0].name;
  document.querySelector(".heading__sub").textContent =
    boardgame.boardgameName[1].name;
  ...
  document.querySelector(
    ".playerNum"
  ).innerHTML = `建議人數：${boardgame.minPlayers}~${boardgame.maxPlayers}位玩家`;
  document.querySelector(
    ".price"
  ).textContent = `建議售價：${boardgame.price}元`;
  document.querySelector(".age").textContent = `建議年齡：${boardgame.ages}+`;
  document.querySelector(
    ".playTime"
  ).textContent = `遊玩時間：${boardgame.playingMins}分鐘`;
  document.querySelector(
    ".weight"
  ).innerHTML = `<p>複雜度：${boardgame.bggWeight}/5 (<a target="_blank" href="${boardgame.bggSite}">BGG</a>)</p>`;

  if (boardgame.cardSleeves.cardNums == 0) {
    sleeves.textContent = `不須牌套`;
    return;
  } else if (
    boardgame.cardSleeves.normalSize == boardgame.cardSleeves.fitSize
  ) {
  ...
  document.querySelector(
    ".publisher"
  ).textContent = `台灣代理商：${boardgame.publisherTW}`;
  document.querySelector(
    ".publishYear"
  ).textContent = `出版年份：${boardgame.publishYear}`;
  let designersStr = "";
  ...
  let salesStr = `<a target="_blank" href ="https://shopee.tw/bakerst01">蝦皮(貝克街)</a> 
  <a target="_blank" href ="https://www.ruten.com.tw/store/gpsam/">露天(貝克街)</a>`;
  document.querySelector(".saleSite").innerHTML = `購買連結：${salesStr}`;
